...
public final class TinyStorage: Sendable {
    ...
    private let dictionaryRepresentation: Mutex<[String: Data]>
    
    let fileWriteMutex: Mutex<Void> = .init(())
    /// Coordinates access to in-memory store
//    private let dispatchQueue = DispatchQueue(label: "TinyStorageInMemory")
    
    private let source: Mutex<DispatchSourceFileSystemObject?> = .init(nil)
    ...
    public var allKeys: [any TinyStorageKey] {
        return dictionaryRepresentation.withLock { dictionaryRepresentation in
            return Array(dictionaryRepresentation.keys)
        }
//        dispatchQueue.sync { return Array() }
    }
    
    /// Subjects for each key that can broadcast specific value changes.
    private let subjects: Mutex<[String: PassthroughSubject<Void, Never>]> = .init([:])
    ...
    public init(insideDirectory: URL, name: String) {
        ...
        self.dictionaryRepresentation = .init(TinyStorage.retrieveStorageDictionary(directoryURL: directoryURL, fileURL: fileURL, logger: logger) ?? [:])
    ...
    public func retrieveOrThrow<T: Codable>(type: T.Type, forKey key: any TinyStorageKey) throws -> T? {
        return try dictionaryRepresentation.withLock { dictionaryRepresentation in
    ...
    public func storeOrThrow(_ value: Codable?, forKey key: any TinyStorageKey) throws {
        ...
        if let value {
            ...
            dictionaryRepresentation.withLock { dictionaryRepresentation in
                ...
                if existingValue != valueData {
                    subjects.withLock { subjects in
                        ...
                        storeToDisk(lockedDictionaryRepresentation: dictionaryRepresentation)
    ...
    private func storeToDisk(lockedDictionaryRepresentation: [String: Data]) {
        guard !Self.isBeingUsedInXcodePreview else { return }
        
        let storageDictionaryData: Data
        
        do {
            storageDictionaryData = try PropertyListSerialization.data(fromPropertyList: lockedDictionaryRepresentation, format: .binary, options: 0)
        } catch {
            logger.error("Error turning storage dictionary into Data: \(error)")
            assertionFailure()
            return
    ...
    private func setUpFileWatch() {
        ...
        self.source.withLock { unkockedSource in
    ...
    private func processFileChangeEvent() {
        ...
        dictionaryRepresentation.withLock { dictionaryRepresentation in
            ...
            if dictionaryRepresentation != newDictionaryRepresentation {
                dictionaryRepresentation = newDictionaryRepresentation
